...
metric_files <- list(
  MPCI = "../Guo_MHBs/chr22/CD4_CD8/MPCI.csv",
  MHL = "../Guo_MHBs/chr22/CD4_CD8/MHL.csv"
)
...
if (length(pca_plots) > 0) {
  ...
  ggsave("../Guo_MHBs/12-13-2025_GUO_classifier_results/CD4_CD8_all_metrics_combined_pca_plots.png", combined_pca_plot, 
...
cat("\n=== STEP 4: RUNNING CLASSIFICATION ANALYSIS (LINEAR KERNEL ONLY) ===\n")

# Enhanced SVM nested CV function with linear kernel only
...
cat("\nMPCI vs MHL Nonparametric Statistical Comparisons:\n")
print(mpci_vs_mhl)

# Save results
write.csv(classification_results, "../Guo_MHBs/12-13-2025_GUO_classifier_results/CD4_CD8_all_metrics_classification_results_linear.csv", row.names = FALSE)
write.csv(stat_test_results, "../Guo_MHBs/12-13-2025_GUO_classifier_results/CD4_CD8_all_metrics_statistical_comparisons_nonparametric.csv", row.names = FALSE)
write.csv(mpci_vs_mhl, "../Guo_MHBs/12-13-2025_GUO_classifier_results/CD4_CD8_mpci_vs_mhl_statistical_comparisons_nonparametric.csv", row.names = FALSE)
...
if (nrow(classification_results) > 0) {
  ...
  auc_bar_plot <- ggplot(auc_summary, aes(x = Metric, y = Mean_AUC)) +
    ...
    labs(title = "AUC Comparison Across Methylation Metrics",
         subtitle = "Linear SVM Kernel Only - Sorted by Performance (Mean ± SE)",
  ...
  ggsave("../Guo_MHBs/12-13-2025_GUO_classifier_results/CD4_CD8_sorted_auc_comparison_se_linear.png", auc_bar_plot, 
  ...
  if (all(c("MPCI", "MHL") %in% classification_results$Metric)) {
    cat("Creating MPCI vs MHL detailed comparison...\n")
    
    # Fix the pivot_longer error by converting to numeric first
    ...
    ggsave("../Guo_MHBs/12-13-2025_GUO_classifier_results/CD4_CD8_mpci_vs_mhl_detailed_comparison_linear.png", mpci_mhl_comparison, 
           width = PARAMS$plot_width-7, height = PARAMS$plot_height-3, dpi = PARAMS$plot_dpi)
    cat("Saved: CD4_CD8_mpci_vs_mhl_detailed_comparison_linear.png\n")
  } else {
    cat("MPCI or MHL not available for comparison plot\n")
  }
} else {
