...
### read pat files
pat_files_path <- "/media/mehrmohammadi_hdd/hdd2/nazer/home/data/loyfer_wgbs/pat file/multi_tissue/"
multi_tissue_pat_files <- paste0(pat_files_path,list.files(pat_files_path))

## generate metrics
## define the metric dfs
MPCI_df <- data.frame(matrix(, nrow = nrow(markers), ncol = length(multi_tissue_pat_files)+1))
names(MPCI_df) <- c("ID", gsub(".*Cap(\\d+).*", "\\1", multi_tissue_pat_files))
MPCI_df$ID <- markers$ID
MHL_df <- data.frame(matrix(, nrow = nrow(markers), ncol = length(multi_tissue_pat_files)+1))
names(MHL_df) <- c("ID", gsub(".*Cap(\\d+).*", "\\1", multi_tissue_pat_files))
MHL_df$ID <- markers$ID
uMHL_df <- data.frame(matrix(, nrow = nrow(markers), ncol = length(multi_tissue_pat_files)+1))
names(uMHL_df) <- c("ID", gsub(".*Cap(\\d+).*", "\\1", multi_tissue_pat_files))
uMHL_df$ID <- markers$ID
dMHL_df <- data.frame(matrix(, nrow = nrow(markers), ncol = length(multi_tissue_pat_files)+1))
names(dMHL_df) <- c("ID", gsub(".*Cap(\\d+).*", "\\1", multi_tissue_pat_files))
dMHL_df$ID <- markers$ID
Beta_df <- data.frame(matrix(, nrow = nrow(markers), ncol = length(multi_tissue_pat_files)+1))
names(Beta_df) <- c("ID", gsub(".*Cap(\\d+).*", "\\1", multi_tissue_pat_files))
Beta_df$ID <- markers$ID

## calculate
for (j in 1:length(multi_tissue_pat_files)){
  fname <- multi_tissue_pat_files[j]
  print(fname)
...
write.csv(MPCI_df, "../random_regions/chr22/multi_tissue/MPCI.csv")
write.csv(Beta_df, "../random_regions/chr22/multi_tissue/Beta.csv")
write.csv(MHL_df, "../random_regions/chr22/multi_tissue/MHL.csv")
write.csv(uMHL_df, "../random_regions/chr22/multi_tissue/uMHL.csv")
write.csv(dMHL_df, "../random_regions/chr22/multi_tissue/dMHL.csv")
