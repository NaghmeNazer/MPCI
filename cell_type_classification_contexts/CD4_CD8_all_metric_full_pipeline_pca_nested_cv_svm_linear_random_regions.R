...
preprocess_metric_data <- function(metric_df) {
  ...
  if (ncol(metric_df) > 1) {
    na_mask <- apply(metric_df[, -1, drop = FALSE], 1, function(x) !any(is.na(x)))
    metric_df_clean <- metric_df[na_mask, ]
  } else {
    metric_df_clean <- metric_df
  }
...
cat("Original vs Clean dimensions:\n")
...
ggsave("../random_regions/11-27-2025_classifier_results/CD4_CD8_all_metrics_combined_pca_plots.png", combined_pca_plot, 
...
# =============================================================================
# STEP 4: NESTED CV FUNCTION WITH PARALLEL PROCESSING
# =============================================================================

cat("\n=== STEP 4: RUNNING CLASSIFICATION ANALYSIS ===\n")

# Enhanced SVM nested CV function
svm_nested_cv <- function(data_matrix, metric_name, kernel_type = "linear") {
...
# Run classification for all metrics and kernels
...
for (metric_name in names(data_matrices)) {
  ...
  # Run linear SVM
  linear_results <- svm_nested_cv(data_matrices[[metric_name]], metric_name, "linear")
...
# =============================================================================
# STEP 6: STATISTICAL ANALYSIS BETWEEN METRICS
# =============================================================================

cat("\n=== STEP 6: STATISTICAL ANALYSIS ===\n")

# Function to perform statistical comparisons
perform_statistical_comparisons <- function(classification_results) {
  test_results <- data.frame()
  
  kernel <- unique(classification_results$Kernel)
  ...
  for(metric in metrics_to_test) {
    for(pair in metric_pairs) {
      ...
      values1 <- classification_results %>% 
          filter(Kernel == kernel & Metric == metric1) %>% 
          pull(!!sym(metric))
        
      values2 <- classification_results %>% 
          filter(Kernel == kernel & Metric == metric2) %>% 
          pull(!!sym(metric))
      ...
      # Perform t-test
      t_test <- t.test(values1, values2, paired = FALSE)
...
stat_test_results <- perform_statistical_comparisons(classification_results)

# Focus on MPCI vs MHL comparisons
mpci_vs_mhl <- stat_test_results %>%
  filter((Metric1 == "MPCI" & Metric2 == "MHL") | (Metric1 == "MHL" & Metric2 == "MPCI")) %>%
  mutate(Comparison = "MPCI_vs_MHL")

cat("\nMPCI vs MHL Statistical Comparisons:\n")
print(mpci_vs_mhl)

# Save results
write.csv(classification_results, "../random_regions/11-27-2025_classifier_results/CD4_CD8_all_metrics_classification_results.csv", row.names = FALSE)
write.csv(stat_test_results, "../random_regions/11-27-2025_classifier_results/CD4_CD8_all_metrics_statistical_comparisons.csv", row.names = FALSE)
write.csv(mpci_vs_mhl, "../random_regions/11-27-2025_classifier_results/CD4_CD8_mpci_vs_mhl_statistical_comparisons.csv", row.names = FALSE)
...
auc_bar_plot <- ggplot(auc_summary, aes(x = Metric, y = Mean_AUC)) +
  ...
  labs(title = "AUC Comparison Across Methylation Metrics",
       subtitle = "Sorted by Performance (Mean ± SE)",
...
ggsave("../random_regions/11-27-2025_classifier_results/CD4_CD8_sorted_auc_comparison_se.png", auc_bar_plot, 
       width = PARAMS$plot_width-7, height = PARAMS$plot_height-3, dpi = PARAMS$plot_dpi)
cat("Saved: CD4_CD8_sorted_auc_comparison_se.png\n")

# 2. MPCI vs MHL detailed comparison with statistical significance
cat("Creating MPCI vs MHL detailed comparison...\n")

# Fix the pivot_longer error by converting AUC to numeric first
mpci_mhl_data <- classification_results %>%
  filter(Metric %in% c("MPCI", "MHL")) %>%
  mutate(
    AUC = as.numeric(AUC),  # Convert AUC to numeric
...
mpci_mhl_summary <- mpci_mhl_data %>%
...
mpci_mhl_comparison <- ggplot(mpci_mhl_summary, aes(x = Metric, y = Mean_Value, fill = Metric)) +
  ...
  labs(title = "MPCI vs MHL Performance Comparison",
       subtitle = "Mean ± Standard Error with Statistical Significance",
       x = "Methylation Metric", 
       y = "Performance Value") +
  scale_fill_manual(values = c("MPCI" = "#1f77b4", "MHL" = "#ff7f0e")) +
  theme_bw() +
  large_text_theme +
...
ggsave("../random_regions/11-27-2025_classifier_results/CD4_CD8_mpci_vs_mhl_detailed_comparison.png", mpci_mhl_comparison, 
       width = PARAMS$plot_width-7, height = PARAMS$plot_height-3, dpi = PARAMS$plot_dpi)
cat("Saved: CD4_CD8_mpci_vs_mhl_detailed_comparison.png\n")
